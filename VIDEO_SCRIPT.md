...
- **Voiceover**:
  ...
  > The goal is to ingest 24-hour campus demand, rooftop solar forecast, and grid tariffs, together with unstructured natural language operator notes, and return both machine-checkable directives and a cost-minimized, 100% compliant 24-hour energy dispatch schedule."
...
- **Voiceover**:
  ...
  > 1. **LLM Interpretation**: We leverage Google Gemini Flash (`gemini-2.5-flash`) with structured output to interpret 1 to 3 operator notes. The LLM extracts the exact directive type, start-inclusive / end-exclusive whole-hour intervals (such as 1 PM to 3 PM mapping to hours 13 and 14), computes solar reduction factors (converting an 80% reduction to a 0.2 multiplier), calculates battery reserves, and accurately filters out distractor notes as `no_op`.
  ...
  > 3. **Mathematical Optimizer**: Finally, the validated directives feed into a SciPy HiGHS Linear Program to compute the globally optimal 24-hour dispatch schedule."
...
- **Voiceover**:
  ...
  > Because HiGHS is an exact LP solver, it guarantees global cost optimality in under 15 milliseconds, achieving a perfect 1.00 quality ratio."

---

### [2:05 - 2:45] Scene 4: Verification & Live Demonstration
- **Visual**: Run terminal command `python test_pipeline.py` showing all 10 sample cases passing with 0 violations and 0.00 cost difference.
- **Voiceover**:
  > "Let's run our comprehensive test pipeline against the 10 public reference cases.
  > As you can see:
  > - `GET /health` responds with `status: ok` immediately.
  > - Malformed JSON safely returns a controlled 400 Bad Request.
  > - All 10 sample cases achieve 100% directive matching, zero energy balance violations, perfect end-of-day neutrality, and match the organizer optimal reference cost down to 0.00 BDT difference.
